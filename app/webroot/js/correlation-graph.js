...
$(document).ready(function() {
    ...
    if (scope === 'event') {
        post["eventid"] = scope_id;
    } else if (scope === 'tag' || scope === 'galaxy') {
        post["tags"] = [scope_id];
    }
    d3.xhr("/events/restSearch.json")
        .header("Content-Type", "application/json")
        .post(
            JSON.stringify(post),
            function(err, rawData) {
                var newData = JSON.parse(rawData.response);
                massageWithNewData(newData);
                update();
            }
        );
    ...
    function massageWithNewData(newData) {
        var aliases = {};
        var added = [];
        if (newData['nodes'] != undefined) {
            newData['nodes'].forEach(function(newNode) {
                var found = false;
                root['nodes'].forEach(function(oldNode, index, rootNodeArray) {
                    if (newNode.unique_id === oldNode.unique_id) {
                        found = true;
                        if (newNode.expanded) {
                            rootNodeArray[index]['expanded'] = true;
                        }
                    } else if (newNode.type === 'attribute' && newNode.name === oldNode.name) {
                        aliases[newNode.unique_id] = index;
                        found = true;
                    }
                });
                if (!found && (newNode.type !== 'attribute' || aliases[newNode.unique_id] == undefined)) {
                    root['nodes'].push(newNode);
                }
            });
        }
        if (newData['links'] != undefined) {
            newData['links'].forEach(function(newLink, index, newDataArray) {
                var foundSource = false;
                var foundTarget = false;
                if (aliases[newLink["source"]] != undefined) {
                    newLink["source"] = aliases[newLink["source"]];
                    foundSource = true;
                }
                if (aliases[newLink["target"]] != undefined) {
                    newLink["target"] = aliases[newLink["target"]];
                    foundTarget = true;
                }
                root['nodes'].forEach(function(oldNode, index2) {
                    if (!foundSource && newLink["source"] === oldNode["unique_id"]) {
                        newLink["source"] = index2;
                        foundSource = true;
                    }
                    if (!foundTarget && newLink["target"] === oldNode["unique_id"]) {
                ...
                if (foundSource && foundTarget) {
                    var foundLink = false;
                    root["links"].forEach(function(oldLink) {
                        if (
                            (newLink["source"] === oldLink["source"] && newLink["target"] === oldLink["target"]) &&
                            (newLink["source"] === oldLink["target"] && newLink["target"] === oldLink["source"])
                        ) {
                            foundLink = true;
                        }
                    });
    ...
    function update() {
        ...
        nodeEnter.insert("circle", ".circle")
            ...
            .attr("r", function(d) {
                return selection_radius_sizes[d.type]
            })
            .attr("stroke", "red")
    ...
    function remove_node(d) {
        ...
        var temp = [];
        root['links'].forEach(function(n, index2, linkArray) {
    ...
    */
    ...
    function expand(d) {
        if (d.type == 'event' || d.type == 'galaxy' || d.type == 'tag') {
            ...
            if (d.type === 'event') {
                post["eventid"] = d.id;
            } else if (d.type === 'tag' || d.type === 'galaxy') {
                post["tags"] = [d.name];
            }
            d3.xhr("/events/restSearch.json")
                .header("Content-Type", "application/json")
...
*/
    ...
    $('#submit_correlation_query').on('click', function() {
        var url = $('#query_url').val().trim();
        var headers = $('#query_headers').val().trim();
        if (headers) {
            var headers_list = [];
            var header_lines = headers.split("\n");
            header_lines.forEach(function(line) {
                line = line.split(":");
                headers_list.push([line[0].trim(), line[1].trim()]);
            });
        } else {
            headers_list = [];
        }
        var body = $('#query_body').val().trim();
        var update_request = d3.xhr(url);
        headers_list.forEach(function(line) {
            update_request.header(line[0], line[1]);
        });
        update_request.post(
            body,
            function(err, rawData) {
